  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-slate-100">{t.bosses_title}</h1>
        <span className="rounded-full bg-slate-800 px-3 py-0.5 text-xs text-slate-400">
          {filtered.length}{t.unit_boss}
        </span>
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder={t.bosses_search_placeholder}
            value={search}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((boss) => (
            <BossCard key={boss.id} boss={boss} t={t} />
          ))}
        </div>
      )}
    </div>
...
function BossCard({ boss, t }: { boss: Boss; t: TranslationDict }) {
  return (
    <div
      ...
      <div className="mb-4 flex items-start justify-between">
        <div className="flex items-center gap-4">
          <ItemImage
            src={boss.image}
            alt={boss.name}
            className="h-20 w-20 shrink-0 rounded-lg border border-slate-700/50"
            placeholderClassName="h-20 w-20 shrink-0 rounded-lg border border-slate-700/50"
            fallback={
